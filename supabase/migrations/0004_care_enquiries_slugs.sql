...
--
-- ---------------------------------------------------------------------------
-- THIS FILE DROPS public.care_enquiries AND EVERYTHING IN IT.
--
-- That is intentional: it was written while the table was empty, to move the
-- five select-backed columns from display labels to stable slugs. Once real
-- enquiries exist, DO NOT RUN THIS. Write an `alter table` migration instead,
-- and update the values in place.
-- ---------------------------------------------------------------------------
--
...
--
-- supabase/care_enquiries.sql and the newest supabase/migrations file are
-- BYTE-IDENTICAL copies. The former is the copy-paste target for the SQL
...
-- 0. Drop the previous shape ---------------------------------------------------
-- Functions first: claim_pending_care_notifications() returns
-- `setof public.care_enquiries`, so it depends on the table's row type and a
-- bare `drop table` would fail. Dropping them explicitly is clearer than
-- `cascade`, which would silently take anything else that happened to depend
-- on the table too.

drop function if exists public.mark_care_notification_sent(
  uuid, boolean, boolean, text
);
drop function if exists public.claim_pending_care_notifications(int, int);
drop function if exists public.submit_care_enquiry(
  text, text, text, text, text, text, text, text, text, text,
  boolean, boolean, text
);
drop table if exists public.care_enquiries;

-- 1. The table ---------------------------------------------------------------
-- The five select-backed columns hold SLUGS, not the text the visitor read.
-- lib/enquiry-options.ts maps slug -> label and is the only place the wording
-- exists; the founder alert resolves them back for display.
--
-- Storing the label instead would couple this table to UI copy: rewording
-- "Bedridden or high care" would need a data migration AND a constraint change
-- AND a redeploy, in that order, or submissions would break in between. It also
-- put U+20B1 and U+2013 into constrained values, where one ASCII lookalike
-- typed into this file would have rejected valid submissions on a single
-- budget band. Slugs are ASCII and permanent.

create table public.care_enquiries (
  ...
  -- 2. Fixed-option constraints ----------------------------------------------
  -- Plain inline CHECKs. The previous version generated these from arrays in a
  -- `do` block and asserted an md5 against the TypeScript copy, because the
  -- constrained values contained a peso sign and an en dash that an editor or a
  -- careless retype could mangle invisibly. The slugs are ASCII, so a mismatch
  -- is now visible in review and the machinery is gone.
  --
  -- scripts/verify-schema.mjs is the real guard: it submits every option in
  -- lib/enquiry-options.ts against the live database and fails on any the
  -- constraints reject. Run it after changing either side.
  --
  -- `care_level is null or ...` is redundant — a CHECK evaluating to UNKNOWN
  -- passes, so NULL would be accepted either way — but stating it stops the
  -- next reader from "fixing" the optional selects into required ones.
  --
  -- parent_location deliberately has NO constraint: it is validated in
  -- TypeScript but is the list most likely to gain a town, and a constraint
  -- would turn that one-line change into a migration.

  constraint care_enquiries_care_type_check check (
    care_type in ('residential_home', 'care_at_home', 'not_sure_yet')
  ),

  constraint care_enquiries_care_level_check check (
    care_level is null or care_level in (
      'mostly_independent', 'needs_daily_help', 'bedridden_or_high_care',
      'dementia_or_memory_care', 'not_sure'
    )
  ),

  constraint care_enquiries_timing_check check (
    timing in ('urgently', 'within_3_months', 'planning_ahead')
  ),

  constraint care_enquiries_budget_band_check check (
    budget_band is null or budget_band in (
      'under_20k', '20k_40k', '40k_70k', 'over_70k', 'not_sure'
    )
  )
);
...
create index care_enquiries_ip_hash_created_at_idx
...
create index care_enquiries_pending_notification_idx
...
begin
  ...
  -- Empty string becomes NULL, meaning "not answered"; a chosen 'not_sure' is a
